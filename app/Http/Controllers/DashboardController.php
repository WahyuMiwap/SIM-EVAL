...
class DashboardController extends Controller
{
    ...
    {
        try {
            $all = LokasiController::allLocations();

            return count(array_filter($all, fn ($l) => in_array(strtolower($l->jenis_sasaran ?? ''), ['sekolah', 'kampus'], true)));
        } catch (\Throwable $e) {
            return 0;
        }
    }
    ...
    public static function calendarEvents(): array
    {
        ...
        try {
            return Event::with(['lokasi', 'participants'])->orderBy('tanggal')->get()->map(function ($e) use ($map) {
            ...
            Log::warning('calendarEvents gagal', ['err' => $e->getMessage()]);
    ...
    protected static function fromEventCollection(iterable $events, callable $getDate, callable $getCount, callable $getGains, callable $getKec, Carbon $now): array
    {
        ...
        foreach ($rows as $r) {
            ...
            if (! isset($kecAgg[$k])) {
                $kecAgg[$k] = ['kecamatan' => $k, 'kegiatan' => 0, 'peserta' => 0];
            }
            $kecAgg[$k]['kegiatan']++;
